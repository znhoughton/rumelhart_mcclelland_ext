...
MIN_WSUP <- 0      # minimum activation-weighted support
...
df_for_model <- function(model_tag,
                                       data_dir = "../Data",
                                       min_len = 1,
                                       min_wsup = 0.7,
                                       alpha = 1.25,
...
df_l3 <- read_csv("../Data/sae_latents_long/L3_H256_SAE@final.csv", show_col_types = FALSE) %>%
    mutate(
      phones = map(present_phones, split_phones),
      activation = as.numeric(activation)
    ) %>%
...
patterns_l3 <- df_l3 %>%
  ...
  group_by(latent_id) %>%
  mutate(
    act_total = sum(df_l3$activation[df_l3$latent_id == first(latent_id)]),
...
extract_patterns_for_model <- function(model_tag,
  ...
  input_csv <- file.path(
    data_dir,
    paste0("sae_latents_long/", model_tag, ".csv")
  )
